...
#define NBD_DEAD_CONN_WAIT 11llu
...
struct nbd_cmd {
	...
	unsigned long flags;
	u32 cmd_cookie;
	struct list_head delayed_entry;
};
...
static unsigned int nbds_max = 32;
						 ...
						 bool reserved)
{
	...
	if (config->num_connections > 1 ||
		...
		if (config->socks) {
			if (cmd->index < config->num_connections) {
				...
				mutex_lock(&nsock->tx_lock);
				/* We can have multiple inflight requests, so
	...
	mutex_unlock(&cmd->lock);
    printk(KERN_DEBUG "nbd: sock_shutdown called from %d\n", __LINE__);
	sock_shutdown(nbd);
	nbd_config_put(nbd);
done:
...
/*
static struct nbd_cmd *nbd_get_cmd(struct nbd_device *nbd, struct nbd_cmd *xcmd)
{
	struct nbd_cmd *cmd, *tmp;

	//mutex_lock(&nbd->delayed_lock);
	list_for_each_entry_safe(cmd, tmp, &nbd->delayed_queue, delayed_entry) {
		if (cmd != xcmd)
			continue;
		list_del_init(&cmd->delayed_entry);
		//mutex_unlock(&nbd->delayed_lock);
		return cmd;
	}
	//mutex_unlock(&nbd->delayed_lock);
	...
	if (rq_data_dir(req) != WRITE) {
		...
		rq_for_each_segment(bvec, req, iter) {
			...
			if (result <= 0) {
				 ...
				 */
				if (nbd_disconnected(config) || !config->dead_conn_timeout) {
					cmd->status = BLK_STS_IOERR;
					goto out;
				}
				ret = -EIO;
...
static void nbd_requeue_delayed_requests(struct nbd_device *nbd)
{
	struct nbd_cmd *cmd, *tmp;
	mutex_lock(&nbd->delayed_lock);
	list_for_each_entry_safe(cmd, tmp, &nbd->delayed_queue, delayed_entry) {
...
static void recv_work(struct work_struct *work)
{
	...
	nbd_requeue_delayed_requests(nbd);
	while (1) {
...
static int wait_for_reconnect(struct nbd_device *nbd)
{
	...
	if (config->dead_conn_timeout) {
		config->dead_conn_timeout -= min(config->dead_conn_timeout, NBD_DEAD_CONN_WAIT);
		msleep(100);
		return 1;
	}
...
//			}
			...
			if (wait_for_reconnect(nbd)) {
				printk(KERN_DEBUG "nbd: wait for socket to reconnect, %lldms left, num_connections=%d, live_connections=%d\n",
					config->dead_conn_timeout, config->num_connections, atomic_read(&config->live_connections));
				goto send;
			}
...
send:
	...
	if (ret == -EAGAIN) {
		...
		mutex_lock(&nbd->delayed_lock);
		list_add_tail(&cmd->delayed_entry, &nbd->delayed_queue);
		mutex_unlock(&nbd->delayed_lock);
	}
out:
	mutex_unlock(&nsock->tx_lock);
			  ...
			  bool netlink)
{
	...
	config->reconnect_timeout = config->dead_conn_timeout;
...
static int nbd_start_device(struct nbd_device *nbd)
{
    ...
    config->dead_conn_timeout = 5000; // 5s
	if (nbd->pid)
        device_remove_file(disk_to_dev(nbd->disk), &pid_attr);
//		return -EBUSY;
	if (!config->socks)
...
static struct nbd_device *nbd_dev_add(int index, unsigned int refs)
{
	...
	mutex_init(&nbd->delayed_lock);
	INIT_LIST_HEAD(&nbd->delayed_queue);
